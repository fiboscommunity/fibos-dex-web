  ...
  const requestForAverageReo = (pairData, toHandleData) => {
    ...
    req
      .callApi('/1.0/app/deal/kdata', 'post', {
        duration: '1d',
        account: repoAccount,
        token_names: pairData.map(({ tokenx, tokeny }) => `${tokeny.id}/${tokenx.id}`),
        stime,
        etime,
      })
      .then(reponse => {
      ...
      tmpReponse.forEach((item, index) => {
        ...
        if (token.id && attributes.market) {
          pairQueue.push({
            ...
            tokenx: new Token({
              id: token.id,
              precision: getPrecision(token.precision),
            }),
            tokeny: new Token({
              id: attributes.market,
            }),
            index,
          })
        }
          ...
          if (res && res.data) {
            ...
            const result = tmpReponse.map((item, index) => {
              ...
              if (Object.keys(totalResData).length > 0) {
                ...
                if (totalResData.tokeny === pairQueue[index].tokenx.id) {
                  needToReverse = true
                }
              ...
              return {
                tokenx: pairQueue[index].tokenx,
                tokeny: pairQueue[index].tokeny,
