...
class RepoTable extends PureComponent {
  ...
  getRepoColumns = (src, dst) => [
    ...
    {
      title: intl.get('REPO_TOKEN'),
      dataIndex: 'direction',
      key: 'direction',
      render: () => {
        const { dstToken, srcToken, needToReverse } = this.props

        if (needToReverse) {
          return dstToken.tokenName || ''
        }

        return srcToken.tokenName || ''
      },
    },
    /* {
    ...
    {
      title: `${intl.get('PRICE')} (${src.symbol})`,
      dataIndex: 'price',
      key: 'price',
      render: (col, row /* , index */) => {
    ...
    },
    {
      ...
      render: (col, row /* , index */) => {
        ...
        if (col) {
          ...
          if (row.direction === 'sell') {
            return `${getStrWithPrecision(new BigNumber(col).plus(row.fee), srcTokenPre)}`
          }
        }
